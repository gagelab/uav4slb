...
#
# Each model is launched in its own named tmux session via run_train_cv0_downsample.sh.
...
#   bash scripts/run_all_models_downsample.sh --parallel --num-gpus 4
#
#   # Auto-detect the number of visible GPUs and round-robin across all of them
#   bash scripts/run_all_models_downsample.sh --parallel --num-gpus auto
#
...
CONFIGS=(
    "configs_downsample/efficientnetv2_s_cv0.yaml"    # CNN  |  ~21 M  | 384×384 | supervised IN-21k
    "configs_downsample/dinov2_vits14_cv0.yaml"        # ViT  |  ~22 M  | 448×448 | DINOv2 self-supervised
    "configs_downsample/maxvit_small_cv0.yaml"         # Hybrid | ~69 M | 224×224 | supervised IN-1k
    "configs_downsample/swinv2_base_cv0.yaml"          # Hybrid | ~88 M | 256×256 | supervised IN-21k
    "configs_downsample/coatnet2_cv0.yaml"             # Hybrid | ~75 M | 224×224 | supervised IN-12k
    "configs_downsample/convnextv2_cv0.yaml"           # CNN  |  ~88 M  | 224×224 | FCMAE self-supervised
    "configs_downsample/convnextv2_large_cv0.yaml"     # CNN  |  ~197 M | 224×224 | FCMAE self-supervised
    "configs_downsample/dinov2_vitb14_cv0.yaml"               # ViT  |  ~86 M  | 518×518 | DINOv2 self-supervised
    "configs_downsample/eva02_base_cv0.yaml"           # ViT  |  ~86 M  | 448×448 | CLIP-MIM merged-38M
)
...
for i in "${!CONFIGS[@]}"; do
    ...
    bash "${SCRIPT_DIR}/run_train_cv0_downsample.sh" \
        --config "${cfg}" \
        "${GPU_ARGS[@]+"${GPU_ARGS[@]}"}" \
...
for cfg in "${CONFIGS[@]}"; do
    ...
    echo "            experiments/${model}_cv0_downsample \\"
done
echo "        --output-dir results/model_comparison_downsample"
echo ""
