"""
figures/fig03_model_comparison_downsample/plot_model_comparison.py
=======================================================
Produces two publication figures from the CV0 (leave-one-year-out) results:
    ...
    Figure 3  — Three-panel aggregate metric bar chart
                ...
                (B) mean test MAE ↓
                (C) mean test RMSE ↓
                Output: fig03_model_comparison_downsample.{pdf,png}

    Figure S4 — Four-panel per-fold test R² bar chart
                One panel per test year (2023, 2024, 2025) plus the mean.
                Output: figures/supplemental/figS4_fold_r2_downsample/figS4_fold_r2_downsample.{pdf,png}
...
-----
    # From repo root:
    python figures/fig03_model_comparison/plot_model_comparison_downsample.py
...
RESULTS_DIR_DEFAULT = ROOT / "downsample_results"
OUTPUT_DIR          = Path(__file__).resolve().parent   # save alongside script
SUPPLEMENTAL_DIR    = ROOT / "figures" / "supplemental" / "figS4_fold_r2_downsample"
...
FOLD_CODES  = ["fold_2023", "fold_2024", "fold_2025"]
FOLD_LABELS = {"fold_2023": "2023", "fold_2024": "2024", "fold_2025": "2025"}
...
def load_summaries(results_dir: Path) -> list[dict]:
    ...
    for summary_path in sorted(results_dir.glob("*/summary.json")):
        ...
        records.append({
            ...
            # per-fold test R² (for Figure S4)
            "test_r2_fold_23":  fold_r2.get("fold_2023", float("nan")),
            "test_r2_fold_24":  fold_r2.get("fold_2024", float("nan")),
            "test_r2_fold_25":  fold_r2.get("fold_2025", float("nan")),
        })
...
# ---------------------------------------------------------------------------
# Figure S4 — per-fold R² bar chart
# ---------------------------------------------------------------------------

def make_fold_r2_figure(records: list[dict]) -> plt.Figure:
    """
    Build the four-panel per-fold test R² figure (Figure S4).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate model comparison figures (Fig 3 and Fig S4).",
    ...
    parser.add_argument(
        ...
        help="Directory where Figure S4 PDF and PNG outputs are written.",
    )
    ...
    save_fig(fig3, out_dir, "fig03_model_comparison_downsample")

    # ── Figure S4 — per-fold R² ───────────────────────────────────────────────
    print("\nBuilding Figure S4 (per-fold R²) …")
    figS3 = make_fold_r2_figure(records)
    save_fig(figS3, args.supplemental_dir, "figS4_fold_r2_downsample")
